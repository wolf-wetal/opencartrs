...
class Confirm extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		...
		if ((!$this->cart->hasProducts() && isset($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$status = false;
		}
      ...
      /*  var_dump($this->session->data['payment_method']);
        var_dump(oc_substr($this->session->data['payment_method']['code'], 0, strpos($this->session->data['payment_method']['code'], '.')));
        die();*/
