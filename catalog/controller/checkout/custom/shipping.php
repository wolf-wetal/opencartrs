...
class Shipping extends \Opencart\System\Engine\Controller {
	...
	public function index($setting) : string {
		...
		if (isset($setting['status']) && (bool)$setting['status'] === true && $this->cart->hasShipping()) {
			...
			$this->load->language('checkout/custom/shipping');
...
/*            var_dump($this->session->data['shipping_method']['code']);
			...
			return $this->load->view('checkout/custom/shipping', $data);
	...
	public function update() : void {
		...
		if (!empty($this->request->get['shipping_method'])) {
			...
			$setting = json_decode($this->model_setting_setting->getValue('module_custom_shipping'), true);
	...
	public function save() : void {
        ...
        $setting = json_decode($this->model_setting_setting->getValue('module_custom_shipping', 0), true);
		...
		if (!$json) {
			...
			// Не авторизован
				...
				if (!$json) {
					...
					$address = $this->model_checkout_custom_cart->addAddressInfo($address);
     ...
     * @return array
     */
	private function getAddress($fields, $post, $shipping_code) : array {
     ...
     * @return array
     */
	private function full($address) : array {
     ...
     * @return bool
     */
	private function validate($name, $value, string $validation = '') : bool {
...
//                var_dump('country_id= ' .$country_id);
//                var_dump('postcode= '.$postcode);
//                var_dump('zone_id= '.$zone_id);
//                var_dump('<pre>');
//                var_dump($this->{'model_extension_' . $result['extension'] . '_shipping_' . $result['code']});
//                die();

                $quote = $this->{'model_extension_' . $result['extension'] . '_shipping_' . $result['code']}->getQuote(array(
...
//                die();
            ...
            /*       var_dump($this->config->get('shipping_'.$result['code'].'_status'));
                  die();*/
/*			if ($this->config->get('shipping_'.$result['code'].'_status')) {
...
/*        var_dump($method_data);
     ...
     * @return array
     */
	private function addAddress($address) : array {
     ...
     * @return array
     */
	private function getAddressById($address_id) : array {
