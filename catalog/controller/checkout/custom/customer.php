...
class Customer extends \Opencart\System\Engine\Controller {
	...
	public function index($setting) : string {
		...
		$this->load->language('checkout/custom/customer');
...
/*        var_dump('<pre>');
		...
		return $this->load->view('checkout/custom/customer', $data);
	...
	public function update() : void {
		...
		$setting = json_decode($this->model_setting_setting->getValue('module_custom_customer'), true);
    ...
    public function save() :void {
		...
		$this->load->model('setting/setting');
 		...
 		$setting = json_decode($this->model_setting_setting->getValue('module_custom_customer', 0), true);
		...
		if (!$this->customer->isLogged()) { 
			...
			if (!$json){
                ...
                // временно, чтобы все время гость был пока без выбора
     ...
     * @return array
     */
    private function full($customer) : array {
     ...
     * @return array
     */
	private function addCustomer($customer) : array {
