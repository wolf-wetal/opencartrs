		...
		if ($product_info) {
			...
			if (isset($this->request->get['path'])) {
				...
				foreach ($parts as $path_id) {
					...
					if ($category_info) {
						$data['breadcrumbs'][] = [
							...
							'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $path). '.html'
						];
					}
				}
				...
				if ($category_info) {
					...
					$data['breadcrumbs'][] = [
						'text' => $category_info['name'],
						'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language')  . '&path=' . $this->request->get['path'] . $url) . '.html'
					];
				}
			}
			...
			if (isset($this->request->get['manufacturer_id'])) {
				$data['breadcrumbs'][] = [
					...
					'href' => $this->url->link('product/manufacturer', 'language=' . $this->config->get('config_language')) . '.html'
				];
				...
				if ($manufacturer_info) {
					$data['breadcrumbs'][] = [
						...
						'href' => $this->url->link('product/manufacturer.info', 'language=' . $this->config->get('config_language') . '&manufacturer_id=' . $this->request->get['manufacturer_id'] . $url) . '.html'
					];
				}
			}
			...
			if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
				...
				$data['breadcrumbs'][] = [
					...
					'href' => $this->url->link('product/search', 'language=' . $this->config->get('config_language') . $url) . '.html'
				];
			}
            ...
            if (isset($this->request->get['path'])) {
                $data['breadcrumbs'][] = [
                    ...
                    'href' => $this->url->addHtmlSuffixToUrls($this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&product_id=' . $product_id . $url))
                ];
            } else {
                $data['breadcrumbs'][] = [
                    'text' => $product_info['name'],
                    'href' => $this->url->link('product/product', 'language=' . $this->config->get('config_language') . $url . '&product_id=' . $product_id) . '.html'
                ];
            }
			...
			if (is_file($k_dir . html_entity_decode($product_info['image'], ENT_QUOTES, 'UTF-8'))) {
			...
			foreach ($results as $result) {
				if (is_file($k_dir . html_entity_decode($result['image'], ENT_QUOTES, 'UTF-8'))) {
					$data['images'][] = [
			...
			foreach ($results as $result) {
				...
				$product_data = [
					...
					'href'        => $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&product_id=' . $result['product_id']) . '.html'
				];
			...
			if ($product_info['tag']) {
				...
				foreach ($tags as $tag) {
					$data['tags'][] = [
						'tag'  => trim($tag),
						'href' => $this->url->link('product/search', 'language=' . $this->config->get('config_language') . '&tag=' . trim($tag)) . '.html'
					];
				}
			}
			...
			$data['advantages_block'] = $this->load->controller('product/advantages_block');
