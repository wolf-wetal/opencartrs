...
class Category extends \Opencart\System\Engine\Controller {
	...
	public function index(): object|null
    {
        ...
        } else
        {
            ...
            $category_id = (int)array_pop($category_path);
            ...
            if ($category_info) {
                ...
                $this->document->setCategoryId($category_id);
                ...
                foreach ($parts as $path_id) {
                    ...
                    $category_info_bea = $this->model_catalog_category->getCategory($path_id);

                    if ($category_info_bea) {
                        $data['breadcrumbs'][] = [
                            'text' => $category_info_bea['name'],
                ...
                if (is_file(DIR_IMAGE . html_entity_decode($category_info['image'], ENT_QUOTES, 'UTF-8'))) {
                    ...
                    $data['image_category'] = $this->config->get('config_url') . 'image/' . html_entity_decode($category_info['image'], ENT_QUOTES, 'UTF-8');
                } else {
                    $data['thumb'] = '';
                    $data['image_category'] = '';
                }
                ...
                foreach ($results as $result) {
                    ...
                    if (is_file(DIR_IMAGE . html_entity_decode($result['icon_category'], ENT_QUOTES, 'UTF-8'))) {
                        $icon_category = $this->model_tool_image->resize(html_entity_decode($result['icon_category'], ENT_QUOTES, 'UTF-8'), $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
                    } else {
                        $icon_category = '';
                    }
                    ...
                    $data['categories'][] = [
                        ...
                        'href'          => $this->url->addHtmlSuffixToUrls($this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $this->request->get['path'] . '_' . $result['category_id'] . $url)),
                ...
                $filter_data = [
                    ...
                    'filter_value_price' => $filter_value_price,
                    'sort' => $sort,
                    'order' => $order,
                    'start' => ($page - 1) * $limit,
                    'limit' => $limit
                ...
                $prices = $this->model_catalog_product->getSumMinAndMaxProducts($filter_data);

                $data['minPrice'] = intval($prices[0]["minPrice"]);
                $data['maxPrice'] = intval($prices[0]["maxPrice"]);
                $data['valuePrice'] = intval($prices[0]["maxPrice"]) / 2;
                ...
                foreach ($results as $result) {
                    ...
                    $product_data = [
                        ...
                        'href' => $this->url->addHtmlSuffixToUrls($this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&product_id=' . $result['product_id'] . $url)),
                    ];
