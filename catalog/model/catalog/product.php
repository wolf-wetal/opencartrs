...
class Product extends \Opencart\System\Engine\Model {
    ...
    public function getProducts(array $data = []): array {
        ...
        if (!empty($data['filter_category_id'])) {
            $sql .=",(SELECT MIN(price) FROM " . DB_PREFIX . "product WHERE product_id IN (SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = " . (int)$data['filter_category_id'] . ") AND status = '1' AND date_available <= NOW()) AS minPrice,
        (SELECT MAX(price) FROM " . DB_PREFIX . "product WHERE product_id IN (SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = " . (int)$data['filter_category_id'] . ") AND status = '1' AND date_available <= NOW()) AS maxPrice";
        ...
        if (!empty($data['filter_value_price'])) {
            $sql .= " AND `p`.`price` <= '" . (float)$data['filter_value_price'] . "'";
        }
        ...
        if (isset($data['sort']) && $data['sort'] == 'p.price') {
            ...
            $sql .= " ORDER BY p.`sort_order` DESC, LCASE(`pd`.`name`) ASC";
        }
    ...
    /**
     * @param int $limit
     * @param int $filter_value_price
     *
     * @return array
     */
    public function getDiscountedProducts(int $limit = 0, int $filter_value_price = 0): array {
        $sql = "SELECT p.*, pd.name, pd.description, p.image, 
            (SELECT MIN(price) FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id) AS discount_price
            ...
            AND p.price > 0
            AND p.product_id IN (SELECT product_id FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id)";

        if (!empty($filter_value_price)) {
            $sql .= " HAVING discount_price <= " . (int)$filter_value_price;
        }
     ...
     * @return array
     */
    public function getDiscountedPriceRange(int $filter_value_price = 0): array {
        $sql = "SELECT 
                MIN((SELECT MIN(price) FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id)) AS minDiscountPrice,
                MAX((SELECT MIN(price) FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id)) AS maxDiscountPrice
            FROM " . DB_PREFIX . "product p
            LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)
            WHERE p.status = '1'
            AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'
        ...
        if ($filter_value_price > 0) {
            $sql .= " HAVING minDiscountPrice <= " . (int)$filter_value_price;
        }
     ...
     * @return array
     */
    public function getDiscountedProductCount(int $filter_value_price = 0): int {
        $sql = "SELECT 
        ...
        if ($filter_value_price > 0) {
            $sql .= " HAVING MIN((SELECT MIN(price) FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id)) <= " . (int)$filter_value_price;
        }

        $query = $this->db->query($sql);

        return (int)$query->row['product_count'];
    }
	...
	public function getTotalProducts(array $data = []): int {
        ...
        if (!empty($data['filter_value_price'])) {
            $sql .= " AND `p`.`price` <= '" . (int)$data['filter_value_price'] . "'";
        }
     ...
     * @return int
     */
    public function getSumMinAndMaxProducts(array $data = []): array {
        $sql = "SELECT MIN(`p`.`price`) AS minPrice, MAX(`p`.`price`) AS maxPrice";
    ...
    public function getSpecialsDiscount(array $data = [], int $filter_value_price = 0): array {
        $sql = "SELECT DISTINCT *, `pd`.`name`, `p`.`image`, `p`.`price`, " . $this->statement['discount'] . ", " . $this->statement['special'] . ", " . $this->statement['reward'] . ", " . $this->statement['review'] . " FROM `" . DB_PREFIX . "product_special` `ps2` LEFT JOIN `" . DB_PREFIX . "product_to_store` `p2s` ON (`ps2`.`product_id` = `p2s`.`product_id` AND `p2s`.`store_id` = '" . (int)$this->config->get('config_store_id') . "' AND `ps2`.`customer_group_id` = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((`ps2`.`date_start` = '0000-00-00' OR `ps2`.`date_start` < NOW()) AND (`ps2`.`date_end` = '0000-00-00' OR `ps2`.`date_end` > NOW()))) LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`p`.`product_id` = `p2s`.`product_id` AND `p`.`status` = '1' AND `p`.`date_available` <= NOW()) LEFT JOIN `" . DB_PREFIX . "product_description` `pd` ON (`pd`.`product_id` = `p`.`product_id`) WHERE `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "' GROUP BY `ps2`.`product_id`";

        if (!empty($filter_value_price)) {
            $sql .= " AND `p`.`price` <= " . (int)$filter_value_price;
        }
     ...
     * @return int
     */
    public function getTotalSpecialsFilter(int $limit = 0, int $filter_value_price = 0): int {
        $sql = "SELECT COUNT(DISTINCT `ps`.`product_id`) AS `total` FROM `" . DB_PREFIX . "product_special` `ps` LEFT JOIN `" . DB_PREFIX . "product_to_store` `p2s` ON (`p2s`.`product_id` = `ps`.`product_id` AND `p2s`.`store_id` = '" . (int)$this->config->get('config_store_id') . "' AND `ps`.`customer_group_id` = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((`ps`.`date_start` = '0000-00-00' OR `ps`.`date_start` < NOW()) AND (`ps`.`date_end` = '0000-00-00' OR `ps`.`date_end` > NOW()))) LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`p2s`.`product_id` = `p`.`product_id` AND `p`.`status` = '1' AND `p`.`date_available` <= NOW())";

        if (!empty($filter_value_price)) {
            $sql .= " WHERE `p`.`price` <= " . (int)$filter_value_price;
        }
...
/*        var_dump($sql);
     ...
     * @return int
     */
    public function getTotalSpecialsFilterCount(int $filter_value_price = 0): int {
        $sql = "SELECT COUNT(DISTINCT `ps2`.`product_id`) AS `total` 
    FROM `" . DB_PREFIX . "product_special` `ps2` 
    LEFT JOIN `" . DB_PREFIX . "product_to_store` `p2s` 
    ON (`ps2`.`product_id` = `p2s`.`product_id` 
    ...
    ON (`ps2`.`product_id` = `p`.`product_id` 
        AND `p`.`status` = '1' 
        AND `p`.`date_available` <= NOW()) 
    ...
    public function getLatestProducts(): array {
        $product_data = $this->cache->get('product.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id'));

        if (!$product_data) {
            $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT 12");

            foreach ($query->rows as $result) {
                $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
            }

            $this->cache->set('product.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') , $product_data);
        }
