<?php
// Heading
$_['heading_title']      = 'Магазин расширений';

// Text
$_['text_success']       = 'Настройки успешно обновлены!';
$_['text_list']          = 'Список расширений';
$_['text_filter']        = 'Фильтр';
$_['text_search']        = 'Поиск';
$_['text_category']      = 'Категории';
$_['text_all']           = 'Все';
$_['text_theme']         = 'Темы';
$_['text_marketplace']   = 'Торговая площадка';
$_['text_language']      = 'Языки';
$_['text_payment']       = 'Оплата';
$_['text_shipping']      = 'Доставка';
$_['text_module']        = 'Модули';
$_['text_total']         = 'Учет в заказе';
$_['text_feed']          = 'Каналы продвижения';
$_['text_report']        = 'Отчеты';
$_['text_other']         = 'Разное';
$_['text_free']          = 'Бесплатно';
$_['text_paid']          = 'Платно';
$_['text_purchased']     = 'Куплено';
$_['text_recommended']   = 'Рекомендуемые';
$_['text_date_modified'] = 'Дата изменения';
$_['text_date_added']    = 'Дата добавления';
$_['text_rating']        = 'Рейтинг';
$_['text_reviews']       = 'отзывов';
$_['text_compatibility'] = 'Совместимость';
$_['text_downloaded']    = 'Загружено';
$_['text_member_since']  = 'Зарегистрирован с :';
$_['text_price']         = 'Цена';
$_['text_featured']      = 'Рекомендуемые';
$_['text_partner']       = 'Разработано OpenCart Партнером';
$_['text_support']       = '12 месяцев бесплатной поддержки';
$_['text_documentation'] = 'Документация включена';
$_['text_sales']         = 'Продаж';
$_['text_comment']       = 'Комментарии';
$_['text_download']      = 'Загрузка';
$_['text_install']       = 'Установка';
$_['text_comment_add']   = 'Оставьте свой комментарий';
$_['text_write']         = 'Напишите свой комментарий тут..';
$_['text_purchase']      = 'Пожалуйста, подтвердите, кто вы!';
$_['text_pin']           = 'Пожалуйста, введите 4 цифры PIN кода. PIN код защищает ваш аккаунт.';
$_['text_secure']        = 'Не сообщайте ваш PIN-код никому, включая разработчиков.';
$_['text_name']          = 'Имя';
$_['text_available']     = 'Доступно для установки';
$_['text_action']        = 'Действие';
$_['text_install']       = 'Установить';
$_['text_uninstall']     = 'Деинсталлировать';
$_['text_delete']        = 'Удалить';
$_['text_more']          = 'смотрите больше ответов...';
$_['text_refresh']       = 'обновить';

// Entry
$_['entry_pin']          = 'PIN';

// Tab
$_['tab_description']    = 'Описание';
$_['tab_documentation']  = 'Документация';
$_['tab_download']       = 'Файлы';
$_['tab_comment']        = 'Комментарии';

// Button
$_['button_api']         = 'API Магазина';
$_['button_purchase']    = 'Купить';
$_['button_view_all']    = 'Посмотреть все расширения';
$_['button_support']     = 'Получить поддержку';
$_['button_comment']     = 'Отправить комментарий';
$_['button_reply']       = 'Ответить';
$_['button_forgot_pin']  = 'Забыл PIN?';

// Error
$_['error_permission']   = 'У Вас нет прав для управления модулем!';
$_['error_api']          = 'Внимание! Вы должны указать ваш OpenCart API до совершения покупки в данном магазине!';
$_['error_purchase']     = 'Расширение не может быть куплено!';
$_['error_download']     = 'Расширение не может быть загружено!';
